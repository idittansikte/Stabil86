local tasks = {
	...
	{questStarted = 1516, questStorage = 65006, killsRequired = 100, raceName = 'Enlightened of the Cult', rewards = {{type = 'exp', values = 3905900}, {type = 'money', values = 1000000}, {type='item', values = {12320, 1}}, {type='item', values = {9693, 1}}, {type='item', values = {5952, 1}}}},
	...
	{questStarted = 1519, questStorage = 65009, killsRequired = 105, raceName = 'Destroyer', rewards = {{type = 'exp', values = 2000000}, {type = 'money', values = 1000000}, {type='item', values = {12320, 1}}, {type='item', values = {5952, 1}}, {type='item', values = {5952, 1}}}},
	...
	{questStarted = 1524, questStorage = 65014, killsRequired = 1500, raceName = 'Giant Spider', rewards = {{type = 'exp', values = 3605100}, {type = 'money', values = 1000000}, {type='item', values = {12320, 1}}, {type='item', values = {12320, 1}}, {type='item', values = {5952, 1}}}},	
	{questStarted = 1525, questStorage = 65015, killsRequired = 230, raceName = 'Hand of Cursed Fate', rewards = {{type = 'exp', values = 9500000}, {type = 'money', values = 1000000}, {type='item', values = {8266, 1}}, {type='item', values = {2492, 1}}, {type='item', values = {5952, 1}}}},	
	...
	{questStarted = 1527, questStorage = 65017, killsRequired = 300, raceName = 'Hellspawn', rewards = {{type = 'exp', values = 2000100}, {type = 'money', values = 1000000}, {type='item', values = {12320, 1}}, {type='item', values = {9693, 1}}, {type='item', values = {5952, 1}}}},	
...
function onSay(cid, words, param, channel)
	if(param == '')then
		doPlayerSendTextMessage(cid, MESSAGE_FIRST, 'There you can see the following tasks, please tell me the number of the task that you want to do. If you are done with your task, write report.')
	...
	elseif(tasks[tonumber(param)])then
		param = tonumber(param)
		if(getCreatureStorage(cid, tasks[param].questStarted) == 1)then
			return doPlayerSendCancel(cid, 'You are already making this task.')
		elseif(getCreatureStorage(cid, tasks[param].questStarted) == 2)then
			return doPlayerSendCancel(cid, 'You already finished this task.')
		elseif(tasks[param].level and getPlayerLevel(cid) < tasks[param].level)then
			return doPlayerSendCancel(cid, 'You need level '..tasks[param].level..' or higher to make this task.')
		end
		for k, v in pairs(tasks) do
			if(getCreatureStorage(cid, v.questStarted) == 1 and tasks[param] ~= k)then
				return doPlayerSendCancel(cid, 'You are already making a task.')
			end
		end
		doCreatureSetStorage(cid, tasks[param].questStarted, 1)
		doCreatureSetStorage(cid, tasks[param].questStorage, 0)
		doPlayerSendTextMessage(cid, MESSAGE_FIRST, 'You have started the task number '..param.. ', remember... in this task you will need to defeat '..tasks[param].killsRequired..' '..tasks[param].raceName..'. Good luck!')
			...
			if(getCreatureStorage(cid, v.questStarted) == 1)then
				if(getCreatureStorage(cid, v.questStorage) >= v.killsRequired)then
					for i = 1, table.maxn(v.rewards) do
						if isInArray({"exp", "experience", 2}, v.rewards[i].type) then
					...
					doPlayerSendTextMessage(cid, MESSAGE_FIRST, 'Great!... you have finished the task number '..k..' Good job.')
					...
					doPlayerSendTextMessage(cid, MESSAGE_FIRST, 'Current '.. getCreatureStorage(cid, v.questStorage)..' '..v.raceName..' killed, you need to kill '..v.killsRequired..'.')
					break
				end
			end
		end
	elseif(param == 'off')then
		for k, v in pairs(tasks) do
			if(getCreatureStorage(cid, v.questStarted) == 1 and tasks[param] ~= k)then
				doCreatureSetStorage(cid, v.questStarted, 0)
				doCreatureSetStorage(cid, v.questStorage, 0)
				doPlayerSendTextMessage(cid, MESSAGE_FIRST, 'You off '..k..' task!')
				break
			end
		end
		return true
