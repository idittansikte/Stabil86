function loseOnZombieArena(cid)
    ...
    if(#players <= 1) then
        ...
        if(winner) then
            doPlayerAddItem(winner, 2157, 20, true)
            doPlayerAddItem(winner, 6119, 1, true)
            doPlayerSendTextMessage(winner, MESSAGE_STATUS_CONSOLE_BLUE, "You won zombies arena event.")
            doBroadcastMessage("||Zombie Event||\nAfter " .. os.time() - getStorage(ZE_ZOMBIES_FIGHT_TIME) .. " seconds of fight " .. getCreatureName(winner) .. " won Zombie Arena Event in game versus " .. getStorage(ZE_ZOMBIES_SPAWNED) .. " zombies!")
            kickPlayerFromZombiesArea(winner)
        else
            doBroadcastMessage("||Zombie Event||\nFinished! No one win?!?!?! WTF!")
        end
